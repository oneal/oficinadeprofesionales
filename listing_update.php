...
//    Condition to get User Id Ends
        ...
        if($src_path == "edit-1"){
            ...
            $email_id = trim(strip_tags(htmlspecialchars($_POST["listing_email"])));

            $register_mode = "Direct";
//            $user_status = "Inactive";

// Common Listing Details
            $listing_name = trim(strip_tags(htmlspecialchars($_POST["listing_name"])));
            $listing_mobile = trim(strip_tags(htmlspecialchars($_POST["listing_mobile"])));
            $listing_email = trim(strip_tags(htmlspecialchars($_POST["listing_email"])));
            $listing_website = trim(strip_tags(htmlspecialchars($_POST["listing_website"])));
            $listing_address = trim(strip_tags(htmlspecialchars($_POST["listing_address"])));
            $listing_lat = trim(strip_tags(htmlspecialchars($_POST["listing_lat"])));
            $listing_lng = trim(strip_tags(htmlspecialchars($_POST["listing_lng"])));
            $listing_description = $_POST["listing_description"];
            $listing_type_id = 1;

            $country_id = 1;
            $service_locations = trim(strip_tags(htmlspecialchars($_POST["service_locations"])));
...
//            }
            ...
            $listing_name_exist_check = mysqli_query($conn, "SELECT * FROM " . TBL . "listings  WHERE listing_slug='" . $listing_slug . "' and listing_id!='" . $listing_id . "'");
...
// Listing Offer Details
            ...
            foreach ($service_1_detail123 as $fruit1)
            {
                ...
                //$service_1_detail = mysqli_real_escape_string($service_1_detail1);
                $prefix1 = '|';
            }
...
// ************************  Offer Image Upload ends  **************************
    ...
    ,service_1_name='" . $service_1_name . "',service_1_price='" . $service_1_price . "', service_1_detail='" . addslashes($service_1_detail) . "'
            ...
            if (count(array_filter($_FILES['gallery_image']['name'])) <= 0) {
                ...
                for ($k = 0; $k<=$num_files; $k++) {
                    if (!empty($_FILES['gallery_image']['name'][$k])) {
                        $file = rand(1000, 100000) . $_FILES['gallery_image']['name'][$k];
...
//    ,gplus_link='" . $gplus_link . "', google_map='" . $google_map . "',360_view='" . $threesixty_view . "'
//    ,service_1_name='" . $service_1_name . "',service_1_price='" . $service_1_price . "', service_1_detail='" . $service_1_detail . "'
//    ,service_1_image='" . $service_1_image . "', service_2_name='" . $service_2_name . "' ,service_2_price='" . $service_2_price . "'
...
//            $to = $admin_email;
//            $subject = $admin_site_name." - ".$BIZBOOK['LISTING_UPDATE_SUCCESS'];
//
...
//            $to1 = $email_id;
//            $subject1 = $admin_site_name." - ".$BIZBOOK['LISTING_UPDATE_SUCCESS'];
//
...
//****************************    client email ends    *************************
            ...
            $src_path_actual = str_replace('edit', 'step', $src_path);
...
//                unset($_SESSION['listing_info_answer']);
                ...
                header('Location: edit-listing-'.$src_path_actual.'?row='.$listing_code);
                exit;
            } else {
...
//            $_SESSION['status_msg'] = "Your Listing has been Updated Successfully!!!";
//
//            header('Location: admin-all-listing');
        } else {
